...
__global__ void GOL (CELL_TYPE *grid, CELL_TYPE *newGrid, int *GPU_lookup_table);
...
int main(int argc, char* argv[])
{
    ...
    cudaMalloc(&GPU_lookup_table, sizeof(int)*2*(CELL_NEIGHBOURS+1));
    ...
    for (iter = 0; iter<MAXITER; iter++) {
        ...
        GOL<<<gridSize, blockSize>>>(d_grid, d_newGrid, GPU_lookup_table); 
