...
typedef  char CELL_TYPE;       // Used for CA grid states
...
__global__ void cyclic_ca (int dim, CELL_TYPE *grid, CELL_TYPE *newGrid)
{
    /*
    ...
    */
    ...
    if (iy>0 && iy < dim+1 && ix>0 && ix < dim+1) {
        cell = grid[id];
        transition = (grid[id+(dim+2)] == (cell+1)%15 || grid[id-(dim+2)] == (cell+1)%15 || grid[id+1] == (cell+1)%15 || grid[id-1] == (cell+1)%15);

        if (cell == STATE1 && transition)
            newGrid[id] = STATE2;
        else if (cell == STATE2 && transition)
            newGrid[id] = STATE3;
        else if (cell == STATE3 && transition)
            newGrid[id] = STATE4;
        else if (cell == STATE4 && transition)
            newGrid[id] = STATE5;
        else if (cell == STATE5 && transition)
            newGrid[id] = STATE6;
        else if (cell == STATE6 && transition)
            newGrid[id] = STATE7;
        else if (cell == STATE7 && transition)
            newGrid[id] = STATE8;

        else if (cell == STATE8 && transition)
            newGrid[id] = STATE9;
        else if (cell == STATE9 && transition)
            newGrid[id] = STATE10;
        else if (cell == STATE10 && transition)
            newGrid[id] = STATE11;
        else if (cell == STATE11 && transition)
            newGrid[id] = STATE12;
        else if (cell == STATE12 && transition)
            newGrid[id] = STATE13;
        else if (cell == STATE13 && transition)
            newGrid[id] = STATE14;
        else if (cell == STATE14 && transition)
            newGrid[id] = STATE15;

        else if (cell == STATE15 && transition)
            newGrid[id] = STATE1;
...
int main(int argc, char* argv[])
{
    ...
    int linGrid = (int)ceil((dim+2)/(float)BLOCK_SIZE);
    ...
    for (iter = 0; iter<maxIter; iter++) {
        ...
        cyclic_ca<<<gridSize, blockSize>>>(dim, d_grid, d_newGrid);
