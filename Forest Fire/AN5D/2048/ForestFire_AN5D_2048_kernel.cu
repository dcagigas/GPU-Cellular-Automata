#include "ForestFire_AN5D_2048_kernel.hu"
...
#define AN5D_TYPE unsigned
    ...
    #define __c0 c0
    const AN5D_TYPE __c1Len = (2048 - 1 + 1);
    const AN5D_TYPE __c1Pad = (1);
    #define __c1 c1
    const AN5D_TYPE __c2Len = (2048 - 1 + 1);
    ...
    #define __LOAD(reg, h) do { if (__loadValid) { __c1 = __c1Pad2 - __halo1 + h; reg = grid[((__c0 % 2) * 2050 + __c1) * 2050 + __c2]; }} while (0)
    #define __DEST (grid[(((c0 + 1) % 2) * 2050 + c1) * 2050 + c2])
    #define __REGREF(reg, i2) reg
    #define __SBREF(sb, i2) __sbref_wrap(sb, (int)__tid + i2)
    #define __CALCEXPR(__rn0, __a, __b, __c) do { __rn0 = lookup_table[grid[((c0 % 2) * 2050 + c1) * 2050 + c2] * 37 + (grid[((c0 % 2) * 2050 + (c1 - 1)) * 2050 + c2] + grid[((c0 % 2) * 2050 + (c1 + 1)) * 2050 + c2] + grid[((c0 % 2) * 2050 + c1) * 2050 + (c2 - 1)] + grid[((c0 % 2) * 2050 + c1) * 2050 + (c2 + 1)])]; } while (0)
