...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
      ...
      unsigned char *dev_lookup_table;
      
      cudaCheckReturn(cudaMalloc((void **) &dev_grid, (size_t)(2) * (size_t)(258) * (size_t)(258) * sizeof(unsigned char)));
      cudaCheckReturn(cudaMalloc((void **) &dev_lookup_table, (size_t)(10) * (size_t)(37) * sizeof(unsigned char)));
      
{
      cudaCheckReturn(cudaMemcpy(dev_grid, grid, (size_t)(2) * (size_t)(258) * (size_t)(258) * sizeof(unsigned char), cudaMemcpyHostToDevice));
#ifdef STENCILBENCH
...
{
      cudaCheckReturn(cudaMemcpy(dev_lookup_table, lookup_table, (size_t)(10) * (size_t)(37) * sizeof(unsigned char), cudaMemcpyHostToDevice));
#ifdef STENCILBENCH
cudaDeviceSynchronize();
SB_START_INSTRUMENTS;
#endif
...
SB_STOP_INSTRUMENTS;
#endif
      cudaCheckReturn(cudaMemcpy(grid, dev_grid, (size_t)(2) * (size_t)(258) * (size_t)(258) * sizeof(unsigned char), cudaMemcpyDeviceToHost));
}
...
int main(int argc, char* argv[])
{
    ...
    #ifdef __VIDEO
        rgb = generate_rgb (SIDE, SIDE, grid, rgb);
	    sprintf (image_name, "%s%d.ppm", "ca_step_", TIMESTEP);
