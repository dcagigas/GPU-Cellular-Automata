...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
      ...
      cudaCheckReturn(cudaMalloc((void **) &dev_grid, (size_t)(2) * (size_t)(16386) * (size_t)(16386) * sizeof(unsigned char)));
      cudaCheckReturn(cudaMalloc((void **) &dev_lookup_table, (size_t)(10) * (size_t)(37) * sizeof(unsigned char)));
      
{
      cudaCheckReturn(cudaMemcpy(dev_grid, grid, (size_t)(2) * (size_t)(16386) * (size_t)(16386) * sizeof(unsigned char), cudaMemcpyHostToDevice));
#ifdef STENCILBENCH
...
SB_STOP_INSTRUMENTS;
#endif
      cudaCheckReturn(cudaMemcpy(grid, dev_grid, (size_t)(2) * (size_t)(16386) * (size_t)(16386) * sizeof(unsigned char), cudaMemcpyDeviceToHost));
}
      for (int c0 = 0; c0 <= 1023; c0 += 1) {
        i = 1;
        for (int c1 = 1; c1 <= 16384; c1 += 1)
          i = (c1 + 1);
      }
      for (int c0 = 0; c0 <= 1023; c0 += 1)
        for (int c1 = 1; c1 <= 16384; c1 += 1) {
          j = 1;
          for (int c2 = 1; c2 <= 16384; c2 += 1)
            j = (c2 + 1);
        }
      t = 0;
      for (int c0 = 0; c0 <= 1023; c0 += 1)
