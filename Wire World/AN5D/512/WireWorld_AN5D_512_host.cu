#include <assert.h>
#include <stdio.h>
#include "WireWorld_AN5D_512_kernel.hu"
#include <stdio.h>
#include <stdlib.h>
...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
      ...
      cudaCheckReturn(cudaMalloc((void **) &dev_grid, (size_t)(2) * (size_t)(512) * (size_t)(512) * sizeof(char)));
      cudaCheckReturn(cudaMalloc((void **) &dev_lookup_table, (size_t)(18) * (size_t)(137) * sizeof(char)));
      
{
      cudaCheckReturn(cudaMemcpy(dev_grid, grid, (size_t)(2) * (size_t)(512) * (size_t)(512) * sizeof(char), cudaMemcpyHostToDevice));
#ifdef STENCILBENCH
...
#define AN5D_TYPE unsigned
      ...
      #define __c0 c0
      const AN5D_TYPE __c1Len = (510 - 1 + 1);
      const AN5D_TYPE __c1Pad = (1);
      #define __c1 c1
      const AN5D_TYPE __c2Len = (510 - 1 + 1);
...
SB_STOP_INSTRUMENTS;
#endif
      cudaCheckReturn(cudaMemcpy(grid, dev_grid, (size_t)(2) * (size_t)(512) * (size_t)(512) * sizeof(char), cudaMemcpyDeviceToHost));
}
      for (int c0 = 0; c0 <= 1023; c0 += 1) {
        i = 1;
        for (int c1 = 1; c1 <= 510; c1 += 1)
          i = (c1 + 1);
      }
      for (int c0 = 0; c0 <= 1023; c0 += 1)
        for (int c1 = 1; c1 <= 510; c1 += 1) {
          j = 1;
          for (int c2 = 1; c2 <= 510; c2 += 1)
            j = (c2 + 1);
        }
      t = 0;
      for (int c0 = 0; c0 <= 1023; c0 += 1)
