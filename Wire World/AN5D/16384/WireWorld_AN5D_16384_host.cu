#include <assert.h>
#include <stdio.h>
#include "WireWorld_AN5D_16384_kernel.hu"
#include <stdio.h>
#include <stdlib.h>
...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
      ...
      char *dev_grid;
      char *dev_lookup_table;
      
      cudaCheckReturn(cudaMalloc((void **) &dev_grid, (size_t)(2) * (size_t)(16384) * (size_t)(16384) * sizeof(char)));
      cudaCheckReturn(cudaMalloc((void **) &dev_lookup_table, (size_t)(18) * (size_t)(137) * sizeof(char)));
      
{
      cudaCheckReturn(cudaMemcpy(dev_grid, grid, (size_t)(2) * (size_t)(16384) * (size_t)(16384) * sizeof(char), cudaMemcpyHostToDevice));
#ifdef STENCILBENCH
...
{
      cudaCheckReturn(cudaMemcpy(dev_lookup_table, lookup_table, (size_t)(18) * (size_t)(137) * sizeof(char), cudaMemcpyHostToDevice));
#ifdef STENCILBENCH
cudaDeviceSynchronize();
SB_START_INSTRUMENTS;
#endif
...
#define AN5D_TYPE unsigned
      ...
      #define __c0 c0
      const AN5D_TYPE __c1Len = (16382 - 1 + 1);
      const AN5D_TYPE __c1Pad = (1);
      #define __c1 c1
      const AN5D_TYPE __c2Len = (16382 - 1 + 1);
      ...
      AN5D_TYPE __side0LenMax;
      {
        ...
        {
          kernel0_4<<<k0_dimGrid, k0_dimBlock>>> (dev_grid, dev_lookup_table, c0);
        }
      }
        ...
        {
          {
            ...
            dim3 k0_dimBlock(__blockSize, 1, 1);
            dim3 k0_dimGrid(1 * ((__c1Len + __side1Len - 1) / __side1Len) * ((__c2Len + __side2Len - 1) / __side2Len), 1, 1);
            kernel0_2<<<k0_dimGrid, k0_dimBlock>>> (dev_grid, dev_lookup_table, c0);
          }
          c0 += 1;
          {
            const AN5D_TYPE __side0Len = 2;
        ...
        {
          {
            ...
            dim3 k0_dimBlock(__blockSize, 1, 1);
            dim3 k0_dimGrid(1 * ((__c1Len + __side1Len - 1) / __side1Len) * ((__c2Len + __side2Len - 1) / __side2Len), 1, 1);
            kernel0_3<<<k0_dimGrid, k0_dimBlock>>> (dev_grid, dev_lookup_table, c0);
          }
          c0 += 1;
          {
            ...
            dim3 k0_dimBlock(__blockSize, 1, 1);
            dim3 k0_dimGrid(1 * ((__c1Len + __side1Len - 1) / __side1Len) * ((__c2Len + __side2Len - 1) / __side2Len), 1, 1);
            kernel0_1<<<k0_dimGrid, k0_dimBlock>>> (dev_grid, dev_lookup_table, c0);
          }
          c0 += 1;
          {
            const AN5D_TYPE __side0Len = 1;
...
SB_STOP_INSTRUMENTS;
#endif
      //cudaCheckReturn(cudaMemcpy(grid, dev_grid, (size_t)(2) * (size_t)(16384) * (size_t)(16384) * sizeof(char), cudaMemcpyDeviceToHost));
      cudaMemcpy(grid, dev_grid, (size_t)(2) * (size_t)(16384) * (size_t)(16384) * sizeof(char), cudaMemcpyDeviceToHost);
}
      for (int c0 = 0; c0 <= 1023; c0 += 1) {
        i = 1;
        for (int c1 = 1; c1 <= 16382; c1 += 1)
          i = (c1 + 1);
      }
      for (int c0 = 0; c0 <= 1023; c0 += 1)
        for (int c1 = 1; c1 <= 16382; c1 += 1) {
          j = 1;
          for (int c2 = 1; c2 <= 16382; c2 += 1)
            j = (c2 + 1);
        }
      t = 0;
      for (int c0 = 0; c0 <= 1023; c0 += 1)
      ...
      //cudaCheckReturn(cudaFree(dev_lookup_table));
      cudaFree(dev_grid);
      cudaFree(dev_lookup_table);
    }
}
...
// 16384 Result in console: initial heads: 33482, final heads: 42706
/*
$ time ./WireWorld_an5d 
Initial Heads: 33482
Final Heads: 42706
