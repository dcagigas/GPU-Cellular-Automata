    ...
    */
    ...
    if ( (iy>0 && iy < GRID_SIZE-1) && (ix>0 && ix < GRID_SIZE-1) ) {
        ...
        if (cell == HEAD) {
            ...
            newGrid[id] = CONDUCTOR;
        } else if (cell == CONDUCTOR) {
            // Get the number of neighbors for a given grid point
            numHeadNeighbors = (grid[id+(GRID_SIZE)]==HEAD) + (grid[id-(GRID_SIZE)]==HEAD)      // upper lower
                     + (grid[id+1]==HEAD) + (grid[id-1]==HEAD)                                  // right left
                     + (grid[id+(GRID_SIZE)+1]==HEAD) + (grid[id-(GRID_SIZE)+1]==HEAD)          // diagonals
                     + (grid[id-(GRID_SIZE)-1]==HEAD) + (grid[id+(GRID_SIZE)-1]==HEAD);
...
int main(int argc, char* argv[])
{
    ...
    size_t bytes = sizeof(CELL_TYPE)*(GRID_SIZE)*(GRID_SIZE);
